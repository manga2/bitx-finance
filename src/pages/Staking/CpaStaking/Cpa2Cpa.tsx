...
import CpaLogo from 'assets/img/token logos/CPA.png';
...
const CPA2CPA = () => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      //     urls: [CPA2CPA_CONTRACT_ABI],
      // });
      // const contract = new SmartContract({
      //     address: new Address(CPA2CPA_CONTRACT_ADDRESS),
      //     abi: new SmartContractAbi(abiRegistry, [CPA2CPA_CONTRACT_NAME]),
      // });
  ...
  React.useEffect(() => {
    if (account.address) {
      axios.get(`${network.apiAddress}/accounts/${account.address}/tokens?search=${CPA_TOKEN_TICKER}`).then((res: any) => {
        let _balance = 0;
        if (res.data?.length > 0) {
          const tokens = res.data.filter(
            (a: any) => a?.identifier === CPA_TOKEN_ID
          );

          if (tokens.length > 0) {
            // console.log('tokens[0]', tokens[0]);
            _balance = convertWeiToEsdt(tokens[0].balance, CPA_TOKEN_DECIMALS);
          }
        }
  ...
  async function stake(e: any) {
    ...
    if (balance == 0) {
      onShowAlertModal(`You don\'t have ${CPA_TOKEN_TICKER} in your wallet.`);
      return;
    }
    ...
    <div className='card'>
      ...
      <div className='info' style={{ marginTop: "8px" }}>
        ...
        <div>
          ...
          <p className='data'>{stakeSetting ? convertAPR2APY(stakeSetting.apr, CPA_CLAIM_PERIOD_IN_DAYS) : '-'} %</p>
        </div>
        <div>
