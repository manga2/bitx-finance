...
import lpadLogo from 'assets/img/token logos/CPA.png';
import coin from 'assets/img/coin.png';
import elrondLogo from 'assets/img/Elrond logo.png';
import AlertModal from 'components/AlertModal';

import {
  CPA2CPA_CONTRACT_ADDRESS,
  CPA2CPA_CONTRACT_ABI,
...
const Cpa2CpaStakingCard = () => {
  ...
  React.useEffect(() => {
    (async () => {
      const registry = await AbiRegistry.load({ urls: [CPA2CPA_CONTRACT_ABI] });
      const abi = new SmartContractAbi(registry, [CPA2CPA_CONTRACT_NAME]);
      const contract = new SmartContract({ address: new Address(CPA2CPA_CONTRACT_ADDRESS), abi: abi });
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const min_stake_limit = convertWeiToEsdt(value.min_stake_limit, CPA_TOKEN_DECIMALS);
      const max_stake_limit = convertWeiToEsdt(value.max_stake_limit, CPA_TOKEN_DECIMALS);
      ...
      const total_staked_amount = convertWeiToEsdt(value.total_staked_amount, CPA_TOKEN_DECIMALS);
      const number_of_stakers = value.number_of_stakers.toNumber();
      const prewithdraw_fee = value.prewithdraw_fee.toNumber() / 100;
      ...
      // console.log('CPA viewStakeSetting', result);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const staked_amount = convertWeiToEsdt(value.staked_amount, CPA_TOKEN_DECIMALS);
      const last_stake_timestamp = value.last_stake_timestamp.toNumber() * SECOND_IN_MILLI;
      const unstaked_amount = convertWeiToEsdt(value.unstaked_amount, CPA_TOKEN_DECIMALS);
      const last_unstake_timestamp = value.last_unstake_timestamp.toNumber() * SECOND_IN_MILLI;
      const collectable_amount = convertWeiToEsdt(value.collectable_amount, CPA_TOKEN_DECIMALS);
      const reward_amount = convertWeiToEsdt(value.reward_amount, CPA_TOKEN_DECIMALS);
      ...
      // console.log('CPA viewStakeAccount', result);
  ...
  React.useEffect(() => {
    if (!address) return;
    getBalanceOfToken(network.apiAddress, account, CPA_TOKEN_ID).then((v) => setBalance(v));
  ...
  function onModalInputAmountChange(v: any) {
    ...
    if (isStakeModal) { // stake
      if (value <= 0) {
        ...
        _modalInfoMesssage = `Cannot stake less than ${stakeSetting.min_stake_limit} ${CPA_TOKEN_TICKER}.`;
      } else if (stakeSetting.max_stake_limit > 0 && stakeAccount.staked_amount + value > stakeSetting.max_stake_limit) {
        _modalInfoMesssage = `Cannot stake more than ${Math.max(stakeSetting.max_stake_limit - stakeAccount.staked_amount, 0)} ${CPA_TOKEN_TICKER}.`;
      } else {
      ...
      if (value <= 0 || value > 100) {
        ...
        _modalInfoMesssage = `20% fee will be deducted if you unstake before ${convertTimestampToDateTime(stakeAccount.last_stake_timestamp + stakeSetting.lock_period)}`;
        _modalButtonDisabled = false;
      } else {
        _modalButtonDisabled = false;
      }
  ...
  async function stake() {
    ...
    const tx = {
      receiver: CPA2CPA_CONTRACT_ADDRESS,
    ...
    <div className='card'>
      ...
      <div className='info' style={{ marginTop: "8px" }}>
        ...
        <div>
          ...
          <p className='data'>{stakeSetting ? stakeSetting.total_staked_amount : '-'} {CPA_TOKEN_TICKER}</p>
        </div>
        <div>
      ...
      <div className='info'>
        <div>
          ...
          <p className='data'>{stakeAccount ? stakeAccount.staked_amount : '-'} CPA</p>
        </div>
        <div>
          <p className='heading'>My Unstaked</p>
          <p className='data'>{stakeAccount ? stakeAccount.unstaked_amount : '-'} CPA</p>
        </div>
        <div>
          <p className='heading'>My Reward</p>
          <p className='data'>{stakeAccount ? stakeAccount.reward_amount : '-'} CPA</p>
        </div>
        <div>
          <p className='heading'>My Collectable</p>
          <p className='data'>{stakeAccount ? stakeAccount.collectable_amount : '-'} CPA</p>
        </div>
      </div>
...
export default Cpa2CpaStakingCard;