...
const Heto2Heto = () => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      //     urls: [HETO2HETO_CONTRACT_ABI],
      // });
      // const contract = new SmartContract({
      //     address: new Address(HETO2HETO_CONTRACT_ADDRESS),
      //     abi: new SmartContractAbi(abiRegistry, [HETO2HETO_CONTRACT_NAME]),
      // });
      // setStakingContract(contract);

      const registry = await AbiRegistry.load({ urls: [HETO2HETO_CONTRACT_ABI] });
      const abi = new SmartContractAbi(registry, [HETO2HETO_CONTRACT_NAME]);
      const contract = new SmartContract({ address: new Address(HETO2HETO_CONTRACT_ADDRESS), abi: abi });
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const min_stake_limit = convertWeiToEsdt(value.min_stake_limit, HETO_TOKEN_DECIMALS);
      ...
      const total_staked_amount = convertWeiToEsdt(value.total_staked_amount, HETO_TOKEN_DECIMALS);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const staked_amount = convertWeiToEsdt(value.staked_amount, HETO_TOKEN_DECIMALS);
      const lock_end_timestamp = value.lock_end_timestamp.toNumber();
      const unstaked_amount = convertWeiToEsdt(value.unstaked_amount, HETO_TOKEN_DECIMALS);
      const undelegation_end_timestamp = value.undelegation_end_timestamp.toNumber();
      const collectable_amount = convertWeiToEsdt(value.collectable_amount, HETO_TOKEN_DECIMALS);
  ...
  React.useEffect(() => {
    if (account.address && !hasPendingTransactions) {
      axios.get(`${network.apiAddress}/accounts/${account.address}/tokens?search=${HETO_TOKEN_TICKER}`).then((res: any) => {
        let _balance = 0;
        if (res.data?.length > 0) {
          const tokens = res.data.filter(
            (a: any) => a?.identifier === HETO_TOKEN_ID
          );

          if (tokens.length > 0) {
            // console.log('tokens[0]', tokens[0]);
            _balance = convertWeiToEsdt(tokens[0].balance, HETO_TOKEN_DECIMALS);
          }
        }
  ...
  function onModalInputAmountChange(value: any) {
    ...
    if (isStakeModal) { // stake
      if (value > balance) {
        ...
        _modalInfoMesssage = `Cannot stake less than ${stakeSetting.min_stake_limit} ${HETO_TOKEN_TICKER}.`;
      } else {
  ...
  async function stake(e: any) {
    ...
    if (balance == 0) {
      onShowAlertModal(`You don\'t have ${HETO_TOKEN_TICKER} in your wallet.`);
      return;
    }

    const args: TypedValue[] = [
      BytesValue.fromUTF8(HETO_TOKEN_ID),
      new BigUIntValue(convertEsdtToWei(modalInputAmount, HETO_TOKEN_DECIMALS)),
    ...
    <div className='card'>
      ...
      <div className='info' style={{ marginTop: "8px" }}>
        ...
        <div>
          ...
          <p className='data'>{stakeSetting ? stakeSetting.total_staked_amount : '-'} {HETO_TOKEN_TICKER}</p>
        </div>
        <div>
      ...
      <div className='info'>
        <div>
          ...
          <p className='data'>{stakeAccount ? stakeAccount.staked_amount : '-'} HETO</p>
        </div>
        <div>
          <p className='heading'>My Unstaked</p>
          <p className='data'>{stakeAccount ? stakeAccount.unstaked_amount : '-'} HETO</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className='data'>{stakeAccount ? stakeAccount.collectable_amount : '-'} HETO</p>
        </div>
      </div>
